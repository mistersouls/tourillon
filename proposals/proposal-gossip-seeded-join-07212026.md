...
**Sequence:** 004
**Revision:** 4
**Updated:** 2026-07-21 — aligned with implemented code: `StatePersistence` (was `StatePort`), `FileStatePersistence` (was `FileStateAdapter`), handlers in `bootstrap/handlers/` (not `core/handlers/`), checks as `NodeStarter` static methods (not `lifecycle/checks.py`), `GossipEngine` takes `partition_shift`, `serializer`, `on_failed`
...
with `phase` in every `FileStatePersistence.save()` call.
...
# tourillon/core/services/starter.py  — static methods on NodeStarter
# (not a separate lifecycle/checks.py module)
@staticmethod
...
# tourillon/bootstrap/handlers/node.py
...
- [ ] All gossip peer-server handlers live in `tourillon/bootstrap/handlers/gossip.py`; node join handler lives in `tourillon/bootstrap/handlers/node.py`.
