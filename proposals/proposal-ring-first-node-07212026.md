...
**Sequence:** 002
**Schema version:** 2
**Updated:** 2026-07-21 — startup phase table expanded (JOINING/DRAINING/PAUSED/FAILED handled); KV socket opened for READY and DRAINING; `NodeStarter` is the bootstrap entry point
...
| `IDLE` (seeds configured) | Peer server bound; waits for `tourctl node join` command. KV server not opened. |
| `READY` | Crash-recovery restart: skip transition, rebuild topology from persisted state, bind both sockets (peer + KV). Gossip bootstrap if seeds configured. |
| `JOINING` | Peer server bound; gossip bootstrap from seeds; waits for rebalance to complete before opening KV. |
| `DRAINING` | Peer + KV server bound; gossip bootstrap from seeds; resumes outgoing transfers. |
| `PAUSED` | Peer server bound only; waits for operator command to resume. No gossip bootstrap. |
| `FAILED` | Peer server bound only; waits for operator command. No gossip. |
...
2026-05-10T14:23:45 INFO     [tourillon.core.services.starter] Node 'node-1' starting from phase 'idle'.
2026-05-10T14:23:45 INFO     [tourillon.core.services.starter] Node 'node-1' bootstrapped as first node: 1024 partitions, 4 vnode(s).
2026-05-10T14:23:45 INFO     [tourillon.core.machinery.state] State persisted to disk (phase: ready, epoch: 1, generation: 1).
...
$ tourillon node start --config config.toml
2026-05-10T14:23:45 INFO     [tourillon.infra.cli.node] Using data directory: /srv/node-1.
2026-05-10T14:23:45 INFO     [tourillon.core.services.starter] Node 'node-1' starting from phase 'ready'.
2026-05-10T14:23:45 INFO     [tourillon.core.services.starter] Topology rebuilt for node 'node-1': 4 vnode(s), epoch 1.
2026-05-10T14:23:45 INFO     [tourillon.core.transport.server] Peer server listening on 0.0.0.0:7701.
2026-05-10T14:23:45 INFO     [tourillon.core.transport.server] KV server listening on 0.0.0.0:7700.
2026-05-10T14:23:45 INFO     [tourillon.infra.cli.node] Node 'node-1' is ready (epoch 1, generation 1).
```
...
#### Seeds configured — seeded start (implemented in proposal 004)

When `seeds` is non-empty in `config.toml`, `tourillon node start` binds the
peer server and waits for the `tourctl node join` command. On join, the node
transitions `IDLE → JOINING`, runs gossip bootstrap against the seeds, and
eventually advances to `READY` via the rebalance protocol (proposal 005).
...
> accesses `state.toml` or `pid.lock` directly. `tourctl node join` connects
> to the peer endpoint over mTLS and triggers the seeded join protocol (defined
> in proposal 004).

> **Note — KV socket lifecycle:** The peer socket (ring / gossip traffic) is
> bound for **every phase** so the node can always receive operator commands.
> The KV socket (data-plane traffic) is bound **only when `phase ∈ {READY,
> DRAINING}`** and closed when the node leaves those phases or stops. It is
> never opened for IDLE, JOINING, PAUSED, or FAILED. KV application handlers
> (`kv.put`, `kv.get`, `kv.delete`) are out of scope for this proposal and
> will be registered by the data-plane layer; until then the KV `Dispatcher`
> has no handlers and closes any incoming connection immediately.
...
state. It is always written atomically via `StatePersistence.save()`. The canonical
...
1. IDLE → READY (single atomic transition — no intermediate JOINING):
   ...
   - Call StatePersistence.save(state) — atomic write + fsync before any socket is
...
`StatePersistence.save()` completes (including fsync) before any gossip or topology
...
- The node state file is **always written before any socket is opened** for any
  phase transition. The peer `TcpServer` is bound only after `StatePersistence.save()`
...
**`StatePersistence` abstracts durable state I/O.**
The bootstrap sequence and phase-transition code call `StatePersistence.save()` rather
than writing TOML directly. This keeps all filesystem and encoding details inside
`FileStatePersistence` in `infra/`, preserving the hexagonal boundary: `core/` never
imports `tomllib`, `pathlib`, or `os`. Tests inject an in-memory `StatePersistence` stub
...
**`FileStatePersistence` is the sole writer of `state.toml`.**
...
class NodeState:
    ...
    exclusively by StatePersistence implementors and the bootstrap sequence; never
...
class StateError(Exception):
    """Raised by StatePersistence implementations on any I/O or decode failure."""
...
# tourillon/core/machinery/state.py
class FileStatePersistence:
    """StatePersistence implementation that reads/writes state.toml in data_dir.
...
tourillon/
  ...
  core/
    ...
    ports/
      state.py        # StatePersistence Protocol, StateError
    ring/
      __init__.py
  ...
  infra/
    store/
      state.py        # FileStatePersistence — StatePersistence impl; temp-file + os.replace() + fsync
    cli/
...
| 10 | In-memory node, `READY` | `run_first_node_bootstrap()` | Returns `NodeState(phase=READY)` unchanged; topology rebuilt from persisted state; `StatePersistence.save()` not called; generation and epoch unchanged |
...
| 16 | `FileStatePersistence`, no file on disk | `load()` | Returns `None` |
| 17 | `FileStatePersistence`, valid `state.toml` with `phase=ready` | `load()` | Returns `NodeState(phase=READY, generation=1, seq=1, tokens=(14,87), epoch=1)` |
| 18 | `FileStatePersistence` | `save(NodeState(...))` then `load()` | Round-trip: loaded state equals saved state |
| 19 | In-memory `StatePersistence` stub | `save()` called from bootstrap sequence | Stub records exactly **one** `save()` call: with `phase=READY`, `epoch=1`, `generation=1` |
| 20 | `FileStatePersistence`, simulate crash mid-write (temp file exists, `os.replace()` not called) | Open adapter fresh and call `load()` | Old state returned; temp file is ignored; no StateError |
...
- [ ] `FileStatePersistence` round-trip test passes (`save` → `load` returns identical `NodeState`).
- [ ] Bootstrap sequence uses `StatePersistence` exclusively; no direct file I/O in `core/`.
- [ ] `tourillon node start` (no seeds) → `READY` in one terminal session with a single `StatePersistence.save()` call; no `JOINING` state is ever persisted.
