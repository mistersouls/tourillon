...
class Ring:
    ...
    def successor(self, token: int) -> VNode:
        ...
        idx = bisect_left(self._vnodes, token, key=lambda v: v.token)
        if idx == len(self._vnodes):
            idx = 0  # Wrap around to the first vnode.
        return self._vnodes[idx]

    def predecessor(self, token: int) -> VNode:
        """Return the first VNode counter-clockwise strictly before token.

        Wraps around to the last vnode when token is at or before the lowest
        token in the ring. Raise ValueError when the ring is empty.
        """
        if not self._vnodes:
