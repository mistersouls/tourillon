...
class HLCTimestamp:
    ...
    @classmethod
    def from_bytes(cls, w: BytesWalker) -> "HLCTimestamp":
        """Decode an HLCTimestamp from *w* and advance the cursor past it.

        Layout: ``wall_ms (8B BE) | counter (2B BE) | nid_len (2B BE) | node_id``.
        Raise ``ValueError`` when the buffer is too short.
        Raise ``UnicodeDecodeError`` when node_id bytes are not valid UTF-8.
        """
        wall_ms = w.read_int(8, "hlc wall_ms")
        counter = w.read_int(2, "hlc counter")
        node_id = w.read_field("hlc node_id").decode("utf-8")
        return cls(wall_ms=wall_ms, counter=counter, node_id=node_id)

    def to_bytes(self) -> bytes:
        """Encode this timestamp to bytes.
        ...
        return (
            self.wall_ms.to_bytes(8, "big")
            + self.counter.to_bytes(2, "big")
            + len(nid).to_bytes(2, "big")
            + nid
        )
